 ...
 * `require("@a5zero7/afyx-graph")` on their OWN Node by re-exporting the
 * compiled library that ships inside the per-platform optionalDependency
 * (@a5zero7/afyx-graph-<target>/lib/dist/index.js).
 *
...
function installPlatformPackage(root: string, sentinel: string): void {
  const pkgRoot = path.join(root, 'node_modules', '@a5zero7', `afyx-graph-${target}`);
  writeFakeLib(path.join(pkgRoot, 'lib', 'dist'), sentinel);
  fs.writeFileSync(
    path.join(pkgRoot, 'package.json'),
    JSON.stringify({ name: `@a5zero7/afyx-graph-${target}`, version: VERSION }) + '\n'
  );
}
...
describe('npm-sdk programmatic entry', () => {
  it('re-exports the installed platform bundle library', () => {
    ...
    const r = requireSdk(mainPkg, { AFYX_GRAPH_INSTALL_DIR: path.join(root, '.empty-cache') });
  ...
  it('falls back to a self-healed cache bundle when the optional dep is absent', () => {
    ...
    const r = requireSdk(mainPkg, { AFYX_GRAPH_INSTALL_DIR: cacheDir });
    expect(r.status).toBe(0);
  ...
  it('throws an actionable error when no bundle is installed or cached', () => {
    ...
    expect(r.stderr).toContain(`@a5zero7/afyx-graph-${target}`);
