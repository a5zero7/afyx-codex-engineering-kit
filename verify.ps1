...
function Test-Skill([string]$Name, [string[]]$RequiredReferences = @()) {
    ...
    foreach ($reference in $RequiredReferences) {
        if (-not (Test-Path -LiteralPath (Join-Path $directory $reference) -PathType Leaf)) { return @{ Ok = $false; Detail = "reference missing: $reference" } }
    }
...
} else { Write-Result 'FAIL' 'Codex CLI' 'not found'; $coreFailure = $true }
...
$checks = @(
    ...
    @{ Name = 'odoo-engineering'; Label = 'Odoo Engineering (10–20)'; Refs = $odooReferences },
...
foreach ($component in @(@{ Label = 'CodeGraph'; Command = 'codegraph' }, @{ Label = 'Headroom'; Command = 'headroom' })) {
