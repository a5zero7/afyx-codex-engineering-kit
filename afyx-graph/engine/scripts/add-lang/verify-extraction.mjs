#!/usr/bin/env node
// Sanity-check that afyx-graph extracted REAL symbols (not just file/import nodes)
...
// Reads `afyx-graph status <repo> --json` using whatever afyx-graph is on PATH,
...
let status;
try {
  const out = execFileSync('afyx-graph', ['status', repo, '--json'], { encoding: 'utf8' });
  status = JSON.parse(out);
} catch (e) {
  console.error(`[verify] could not read afyx-graph status for ${repo}: ${e.message}`);
  process.exit(2);
}

// Kinds that prove the extractor mapped AST node types (everything except
// 'file' and 'import', which afyx-graph creates structurally for any language).
