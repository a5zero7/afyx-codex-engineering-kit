...
#   clone -> wipe+index (with the afyx-graph on PATH) -> verify extraction ->
#   with/without retrieval A/B (reuses scripts/agent-eval/run-all.sh).
#
# Assumes the afyx-graph dev build is already built + linked on PATH — the skill
...
# Env:   CORPUS   corpus dir (default /tmp/afyx-graph-corpus, shared with agent-eval)
set -uo pipefail
...
CORPUS="${CORPUS:-/tmp/afyx-graph-corpus}"
REPO="$CORPUS/$NAME"

command -v afyx-graph >/dev/null || { echo "no afyx-graph on PATH (build + ./scripts/local-install.sh first)"; exit 1; }

echo "==================== add-lang bench: $NAME ($LANG_TOKEN) ===================="
echo "afyx-graph: $(command -v afyx-graph) -> $(afyx-graph --version 2>/dev/null || echo '?')"
...
echo "→ wiping .afyx-graph and indexing"
rm -rf "$REPO/.afyx-graph"
( cd "$REPO" && afyx-graph init -i ) || { echo "indexing failed"; exit 1; }
