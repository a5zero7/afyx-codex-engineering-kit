 ...
 *     `./.kiro/settings/mcp.json` (local). Standard `mcpServers.afyx_graph`
 *     shape, same as Claude / Cursor / Gemini.
 *   - Instructions to `~/.kiro/steering/afyx-graph.md` (global) or
 *     `./.kiro/steering/afyx-graph.md` (local). Kiro's "steering" system
 *     loads every `*.md` file in the steering dir as agent context, so
 *     a dedicated `afyx-graph.md` is the natural surface — we own the
...
function steeringPath(loc: Location): string {
  return path.join(configDir(loc), 'steering', `${CLI_NAME}.md`);
}
...
class KiroTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    // The steering doc is no longer written — the afyx-graph usage
    // guidance ships in the MCP server's `initialize` response (issue
    // #529). Delete a `afyx-graph.md` a previous install created so an
    // upgrade self-heals.
  ...
  printConfig(loc: Location): string {
    ...
    const snippet = JSON.stringify({ mcpServers: { [MCP_SERVER_NAME]: getMcpServerConfig() } }, null, 2);
 ...
 * out of recognition we still remove it — afyx-graph.md is a name we
