 ...
 * was then auto-migrated by Antigravity, and re-ran `afyx-graph install`
 * doesn't end up with stale afyx-graph entries in two files.
 *
 ...
 * to get afyx-graph to appear in the Customizations UI. We build the
 ...
 * tools live outside that, so a bare `afyx-graph` command fails to spawn
 * even when `which afyx-graph` resolves in the user's shell. We resolve
 * `afyx-graph` to its absolute path on macOS at install time. (Linux GUI
 ...
 * still get a working MCP integration; the prefer-afyx-graph-over-grep
...
/**
 * Resolve the on-disk path of the `afyx-graph` binary so a Mac GUI app
 * launched from Dock/Finder (with a stripped PATH) can find it. Falls
 * back to the bare `afyx-graph` name when:
 *
 ...
 */
function resolveAfyxGraphCommand(): string {
  if (process.platform !== 'darwin') return CLI_NAME;
  try {
    const resolved = execSync(`command -v ${CLI_NAME} || which ${CLI_NAME}`, {
      encoding: 'utf-8',
...
/**
 * Build the afyx-graph MCP-server entry for Antigravity. Distinct from
...
function buildAntigravityEntry(): { command: string; args: string[] } {
  return {
    command: resolveAfyxGraphCommand(),
    args: ['serve', '--mcp'],
  };
}
...
class AntigravityTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    // wind up with two competing afyx-graph configs.
  ...
  uninstall(loc: Location): WriteResult {
    ...
    files.push(removeAfyxGraphFromFile(preferred));

    // Also sweep the OTHER path (legacy when preferred is unified, and
    // vice versa) — handles the migration-half-state case where afyx-graph
    ...
    if (preferred !== other) {
      const otherResult = removeAfyxGraphFromFile(other);
  ...
  printConfig(loc: Location): string {
    ...
    const file = preferredMcpConfigPath();
    const snippet = JSON.stringify({ mcpServers: { [MCP_SERVER_NAME]: buildAntigravityEntry() } }, null, 2);
    return `# Add to ${file}\n\n${snippet}\n`;
  }
...
/**
 * Strip the afyx-graph entry from the legacy `~/.gemini/antigravity/mcp_config.json`
 * if it's present AND we're writing to the unified path. Used by install
 * to migrate users who had afyx-graph configured on the legacy path
...
function removeAfyxGraphFromFile(file: string): WriteResult['files'][number] {
  if (!fs.existsSync(file)) return { path: file, action: 'not-found' };
  const config = readJsonFile(file);
  if (!config.mcpServers?.[MCP_SERVER_NAME]) return { path: file, action: 'not-found' };
  delete config.mcpServers[MCP_SERVER_NAME];
  if (Object.keys(config.mcpServers).length === 0) {
    delete config.mcpServers;
  }
