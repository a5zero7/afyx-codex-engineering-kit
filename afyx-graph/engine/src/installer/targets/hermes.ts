 ...
 *
 *   mcp_servers.afyx_graph -> `afyx-graph serve --mcp`
 *   platform_toolsets.cli -> `mcp-afyx_graph`
 *
 * The second entry matters because Hermes CLI profiles often enable an
 * explicit `platform_toolsets.cli` list. Without `mcp-afyx_graph` in that
...
class HermesTarget implements AgentTarget {
  ...
  detect(loc: Location): DetectionResult {
    ...
    return {
      installed,
      alreadyConfigured: hasAfyxGraphMcpServer(content),
      configPath: file,
    };
  }
  ...
  uninstall(loc: Location): WriteResult {
    ...
    const after = removeAfyxGraphToolset(removeAfyxGraphMcpServer(before));
    if (after === before) {
      return { files: [{ path: file, action: 'not-found' }] };
    }
...
function writeHermesConfig(): WriteResult['files'][number] {
  ...
  const before = readText(file);
  const afterMcp = upsertAfyxGraphMcpServer(before);
  const after = upsertAfyxGraphToolset(afterMcp);
...
function renderAfyxGraphMcpChild(): string[] {
  return [
    `  ${MCP_SERVER_NAME}:`,
    `    command: ${CLI_NAME}`,
    '    args:',
...
function renderAfyxGraphMcpBlock(): string[] {
  return ['mcp_servers:', ...renderAfyxGraphMcpChild()];
}

function hasAfyxGraphMcpServer(content: string): boolean {
  const lines = splitLines(content);
  const parent = topLevelRange(lines, 'mcp_servers');
  return !!parent && !!childRange(lines, parent, MCP_SERVER_NAME);
}

function upsertAfyxGraphMcpServer(content: string): string {
  const lines = splitLines(content);
  const parent = topLevelRange(lines, 'mcp_servers');
  const child = parent ? childRange(lines, parent, MCP_SERVER_NAME) : null;
  const replacement = renderAfyxGraphMcpChild();

  if (!parent) {
    if (lines.length > 0 && lines[lines.length - 1] === '') lines.pop();
    if (lines.length > 0) lines.push('');
    lines.push(...renderAfyxGraphMcpBlock());
...
function removeAfyxGraphMcpServer(content: string): string {
  const lines = splitLines(content);
  const parent = topLevelRange(lines, 'mcp_servers');
  const child = parent ? childRange(lines, parent, MCP_SERVER_NAME) : null;
...
function upsertAfyxGraphToolset(content: string): string {
  const lines = splitLines(content);
  ...
  if (!parent) {
    if (lines.length > 0 && lines[lines.length - 1] === '') lines.pop();
    if (lines.length > 0) lines.push('');
    lines.push('platform_toolsets:', '  cli:', '    - hermes-cli', '    - mcp-afyx_graph');
    return joinLines(lines);
  }

  if (!cli) {
    lines.splice(parent.end, 0, '  cli:', '    - hermes-cli', '    - mcp-afyx_graph');
    return joinLines(lines);
  }

  const hasEntry = lines
    .slice(cli.start + 1, cli.end)
    .some((line) => line.trim() === '- mcp-afyx_graph');
  if (hasEntry) return joinLines(lines);

  lines.splice(cli.end, 0, `${cli.itemIndent}- mcp-afyx_graph`);
  return joinLines(lines);
}

function removeAfyxGraphToolset(content: string): string {
  const lines = splitLines(content);
  const parent = topLevelRange(lines, 'platform_toolsets');
  const cli = parent ? listChildBlock(lines, parent, 'cli') : null;
  ...
  const next = lines.filter((line, idx) => {
    ...
    return line.trim() !== '- mcp-afyx_graph';
  });
