/**
 * Project-scoped configuration: a committed `afyx-graph.json` at the project
 * root that a team shares through version control.
 *
 * Today it carries one thing — `extensions`, an opt-in map from a custom file
 * extension to one of Afyx Graph's supported languages. The built-in
...
export const PROJECT_CONFIG_FILENAME = 'afyx-graph.json';
...
export interface ProjectConfig {
   ...
   * Afyx Graph's own data dir are never resurfaced; an explicit `exclude` still
...
/** Parsed, validated view of a project's `afyx-graph.json`. */
 ...
 * `stat` while a single `afyx-graph.json` is in force. Keying by root keeps two
...
/**
 * Read + JSON-parse a `afyx-graph.json` once and return its validated view.
...
/**
 * Load the parsed `afyx-graph.json` for a project, mtime-cached. A missing or
 ...
 * map when there is no `afyx-graph.json` (the zero-config default).
 */
 ...
 * nothing in. Built-in default-ignored dirs, `.git`, and Afyx Graph's data dir
...
/**
 * Add gitignore-style patterns to a project's `afyx-graph.json` `includeIgnored`
 ...
 *
 * A plain-JSON round-trip: a `afyx-graph.json` carrying comments (not valid JSON)
