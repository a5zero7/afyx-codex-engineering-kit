 ...
 * reliably crashes Afyx Graph with `Fatal process out of memory: Zone`
...
/**
 * Build the bordered banner shown when Afyx Graph detects an
...
export function buildNode25BlockBanner(nodeVersion: string): string {
  ...
  return [
    ...
    'crashes with `Fatal process out of memory: Zone` when Afyx Graph',
    'compiles tree-sitter grammars. Afyx Graph WILL crash on this Node',
