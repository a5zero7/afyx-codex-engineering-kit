 ...
 *    canonical `hook_X` name, linking implementations to the hook when `afyx_graph_callers`
 *    is invoked.
 *
 ...
 *   `afyx_graph_search("form_alter")`. Full hook-node creation (virtual nodes for every hook)
 ...
 * - TODO: Improve hook resolution: create virtual `hook_*` nodes so `afyx_graph_callers`
