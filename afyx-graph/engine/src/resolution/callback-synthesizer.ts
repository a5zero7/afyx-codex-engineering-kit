 ...
 * its struct). That breaks `afyx_graph_node` member outlines, any
...
export async function synthesizeCallbackEdges(
  ...
  // Per-pass wall-clock timing to stderr, opt-in via AFYX_GRAPH_SYNTH_TIMINGS
  ...
  const __mark = (label: string): void => {
    ...
    markT.t = now;
    if (process.env.AFYX_GRAPH_SYNTH_TIMINGS && (dt > 250 || process.env.AFYX_GRAPH_SYNTH_TIMINGS === 'all')) {
