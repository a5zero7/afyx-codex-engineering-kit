 ...
 * @example
 * ```typescript
 * import { FileError, ParseError, setLogger, silentLogger } from 'afyx-graph';
 *
...
/**
 * Base error class for all Afyx Graph errors.
 *
 * All Afyx Graph-specific errors extend this class, allowing you to catch
 * all Afyx Graph errors with a single catch block.
 *
 * @example
 * ```typescript
 * try {
 *   await cg.indexAll();
 * } catch (error) {
 *   if (error instanceof AfyxGraphError) {
 *     console.log(`Afyx Graph error [${error.code}]: ${error.message}`);
 *   }
 * }
 * ```
 */
export class AfyxGraphError extends Error {
 ...
 */
export class FileError extends AfyxGraphError {
 ...
 */
export class ParseError extends AfyxGraphError {
  readonly filePath: string;
 ...
 */
export class DatabaseError extends AfyxGraphError {
 ...
 */
export class SearchError extends AfyxGraphError {
 ...
 */
export class VectorError extends AfyxGraphError {
  constructor(message: string, operation: string, cause?: Error) {
 ...
 */
export class ConfigError extends AfyxGraphError {
 ...
 * {@link AfyxGraphError}: it is a security marker every read sink tests with
...
/**
 * Simple logger for Afyx Graph operations
 *
...
export const defaultLogger: Logger = {
  debug(message: string, context?: Record<string, unknown>): void {
    if (process.env.AFYX_GRAPH_DEBUG) {
      console.debug(`[Afyx Graph] ${message}`, context ?? '');
    }
  },
  warn(message: string, context?: Record<string, unknown>): void {
    console.warn(`[Afyx Graph] ${message}`, context ?? '');
  },
  error(message: string, context?: Record<string, unknown>): void {
    console.error(`[Afyx Graph] ${message}`, context ?? '');
  },
};
