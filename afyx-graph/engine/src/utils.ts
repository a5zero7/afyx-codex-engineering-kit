 ...
 * @example
 * ```typescript
 * import { Mutex, processInBatches, MemoryMonitor, validatePathWithinRoot } from 'afyx-graph';
 *
 ...
 * API key, JDBC URL with embedded creds). Afyx Graph must surface the KEY only
 ...
 * (afyx_graph_node `includeCode`, afyx_graph_explore source) go through here, so
  ...
  acquire(): void {
    ...
    if (fs.existsSync(this.lockPath)) {
      try {
        ...
        if (lockAge < FileLock.STALE_TIMEOUT_MS && !isNaN(pid) && this.isProcessAlive(pid)) {
          throw new Error(
            `Afyx Graph database is locked by another process (PID ${pid}). ` +
            `If this is stale, run 'afyx-graph unlock' or delete ${this.lockPath}`
          );
        }
    ...
    try {
      ...
      if (err.code === 'EEXIST') {
        ...
        throw new Error(
          'Afyx Graph database is locked by another process. ' +
          `If this is stale, run 'afyx-graph unlock' or delete ${this.lockPath}`
        );
      }
      throw err;
