 ...
 * `node:sqlite`. `afyx_graph_explore` is CPU-heavy (FTS + RWR/personalized-
 ...
 * worktree), and `afyx_graph_status`, none of which a watcher-less
...
// Mirror the engine's lazy-require of the heavy Afyx Graph + tools chain. This
...
if (parentPort) {
  ...
  try {
    const cg = loadAfyxGraph().openSync(root);
  ...
  const serve = async (msg: CallMessage): Promise<void> => {
    ...
    if (msg.toolName === '__test_crash__' && process.env.AFYX_GRAPH_QUERY_WORKER_ALLOW_TEST_CRASH === '1') {
      process.exit(13);
    }
    if (!handler) {
      port.postMessage({
        type: 'result',
        id: msg.id,
        result: errorResult(`afyx-graph worker could not open the project: ${initError}`),
      });
      return;
    }
    try {
