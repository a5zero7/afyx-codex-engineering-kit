...
export class ContextBuilder {
  ...
  /**
   * Whether the project's `afyx-graph.json` `deprioritize` patterns cover this
  ...
  private buildLowConfidenceNote(entryPoints: Node[]): string {
    ...
    const dirLine = dirs.length
      ? `\n- \`afyx_graph_files\` a likely area: ${dirs.map(d => `\`${d}\``).join(', ')}`
      : '';
   ...
   * truncation is honest, and the agent can afyx_graph_node the last hop to bridge.
   */
