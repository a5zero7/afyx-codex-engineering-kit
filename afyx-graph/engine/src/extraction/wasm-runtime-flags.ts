...
export function nodeRuntimeFlagsFor(nodeVersion: string): readonly string[] {
  ...
  // a fatal "bad option" at spawn. Those runtimes can't run afyx-graph anyway
 ...
 */
const RELAUNCH_GUARD_ENV = 'AFYX_GRAPH_WASM_RELAUNCHED';
 ...
 * disabled via AFYX_GRAPH_NO_RELAUNCH.
 *
...
export function relaunchWithWasmRuntimeFlagsIfNeeded(scriptPath: string): void {
  ...
  if (process.env.AFYX_GRAPH_NO_RELAUNCH) return;
