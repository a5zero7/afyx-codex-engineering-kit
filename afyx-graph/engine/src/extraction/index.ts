...
export interface IndexResult {
   ...
   * user — or an agent — that there was code here Afyx Graph could not read
 ...
 * languages/frameworks Afyx Graph supports — curated from the canonical
 ...
 * — IDE/state dirs like `.idea`/`.vs` are omitted because Afyx Graph indexes only
 ...
 * slowing indexing, and skewing both the file count and `afyx_graph_explore`
...
/**
 * Matcher for the project's `afyx-graph.json` `includeIgnored` patterns — the
...
/**
 * Matcher for the project's `afyx-graph.json` `exclude` patterns — paths to keep
...
/**
 * Matcher for the project's `afyx-graph.json` `include` patterns — first-party
 ...
 * Afyx Graph's data dir — those are never resurfaced, mirroring `ScopeIgnore`.
...
function collectIncludedFiles(
  ...
  function walk(absDir: string): void {
    ...
    for (const entry of entries) {
      if (entry.name === '.git' || isAfyxGraphDataDir(entry.name)) continue;
...
/**
 * The included source files (`afyx-graph.json` `include`) for a scan root, or an
 ...
 *   of a repo Afyx Graph already indexes — indexing it just duplicates the whole
 ...
 * from npm git-dependencies — that never makes it project code) and Afyx Graph
 ...
 *      NOT opt that path in via `afyx-graph.json` `includeIgnored`. The gitignore
 ...
 * (#514) only for directories the project opted in via `afyx-graph.json`
 ...
 * opted into via `afyx-graph.json` `includeIgnored`. These are real repos the
 ...
 * tool (and Afyx Graph's own `git ls-files` foundation) does — `.gitignore`
 * excludes. So this returns `[]` unless the project opted the directory in via
 * `afyx-graph.json` `includeIgnored`; without that, a gitignored dir — including
 ...
 * `afyx-graph.json` include allowlist silently stopped applying and files went
 ...
 * directories the project opted in through `afyx-graph.json` `includeIgnored`
...
function collectGitFiles(repoDir: string, prefix: string, files: Set<string>, embeddedRoots?: Set<string>, includeIgnored: Ignore | null = null): void {
  ...
  // only when the project opted the directory in via `afyx-graph.json`
...
function getGitVisibleFiles(rootDir: string): Set<string> | null {
  try {
    ...
    // `afyx-graph.json` `include`. These are gitignored, so `git ls-files` never
    ...
    // existing AFYX_GRAPH_DEBUG gate so a future report can confirm or rule out
 ...
 * directories opted in via `afyx-graph.json` `includeIgnored` (#622, #699) —
 ...
 * with it); a full `afyx-graph index` reconciles that.
 */
export function getGitChangedFiles(rootDir: string, sinceCommit?: string | null): GitChanges | null {
  try {
    ...
    // Custom extension → language overrides from the project's afyx-graph.json,
...
function collectGitStatus(repoDir: string, prefix: string, out: GitChanges, overrides?: Record<string, Language>, includeIgnored: Ignore | null = null, exclude: Ignore | null = null, sinceCommit?: string): void {
  ...
  // change — so `afyx-graph status` (which reads getChangedFiles) reports a
  ...
  const classify = (statusCode: string, rel: string): void => {
    ...
    // User `afyx-graph.json` `exclude` (#999) is project-root-relative, so it's
...
function scanDirectoryWalk(
  ...
  function walk(dir: string, matchers: ScopedIgnore[]): void {
    ...
    for (const entry of entries) {
      // Never descend into git internals or any Afyx Graph data directory
  ...
  // Project `afyx-graph.json` `exclude` patterns (#999), rooted at the project so
  ...
  // Force-include first-party source whitelisted in `afyx-graph.json` `include`
...
export class ExtractionOrchestrator {
   ...
   * files it is derived from (`afyx-graph.json`, `.gitignore`). See
   ...
   * that is OUT of scope (a user `exclude` in `afyx-graph.json`, a `.gitignore`
  ...
  async indexAll(
    ...
    if (
      storeWriterOpts &&
      process.env.AFYX_GRAPH_NO_STORE_WORKER !== '1' &&
  ...
  async indexFileWithContent(
    ...
    // Detect language (honoring the project's afyx-graph.json extension overrides)
   ...
   * correct against an older graph. Measured on afyx-graph's own long-lived
  ...
  async sync(
    ...
    if (scopedPaths && scopedPaths.length > 0) {
      ...
      // when every scoped path was a deletion, because AfyxGraph.watch()
      ...
      // user had just excluded in `afyx-graph.json` while `afyx-graph sync`
      ...
      if (process.env.AFYX_GRAPH_SYNTH_TIMINGS) console.error(`[phase-timing] sync-scoped: ${Date.now() - tSyncScan}ms (${unique.length} paths, ${trackedFiles.length} tracked)`);
    } else {
      ...
      if (process.env.AFYX_GRAPH_SYNTH_TIMINGS) console.error(`[phase-timing] sync-scan: ${Date.now() - tSyncScan}ms (${currentFiles.length} files)`);
