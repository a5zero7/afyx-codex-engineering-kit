/**
 * afyx_graph_node FILE READ mode: a `file` with no `symbol` reads that file like
...
describe('afyx_graph_node file-view (Read replacement)', () => {
  let dir: string;
  let cg: AfyxGraph;
  let h: ToolHandler;
  ...
  beforeEach(async () => {
    ...
    cg = AfyxGraph.initSync(dir, { config: { include: ['**/*.ts', '**/*.properties'], exclude: [] } });
    await cg.indexAll();
  ...
  const text = async (args: Record<string, unknown>): Promise<string> =>
    (await h.execute('afyx_graph_node', args)).content.map((c) => c.text).join('\n');
