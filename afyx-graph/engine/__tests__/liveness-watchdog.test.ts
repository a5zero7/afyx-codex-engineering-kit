...
describe('installMainThreadWatchdog opt-out', () => {
  it('returns null (spawns nothing) when AFYX_GRAPH_NO_WATCHDOG is set', () => {
    const prev = process.env.AFYX_GRAPH_NO_WATCHDOG;
    process.env.AFYX_GRAPH_NO_WATCHDOG = '1';
    try {
...
describe('liveness watchdog (spawned, real watchdog process)', () => {
  ...
  it('SIGKILLs a process whose main thread wedges in a sync loop', async () => {
    const r = await runChild(
      { AFYX_GRAPH_WATCHDOG_TIMEOUT_MS: '500' },
  ...
  it('kills at the hard cap even with ongoing file activity (bounded deferral)', async () => {
    ...
    const [r] = await Promise.all([
      runChild(
        { AFYX_GRAPH_WATCHDOG_TIMEOUT_MS: '300' },
  ...
  it('does NOT kill a wedged process when AFYX_GRAPH_NO_WATCHDOG=1', async () => {
    const { code, signal } = await runChild(
      { AFYX_GRAPH_WATCHDOG_TIMEOUT_MS: '500', AFYX_GRAPH_NO_WATCHDOG: '1' },
