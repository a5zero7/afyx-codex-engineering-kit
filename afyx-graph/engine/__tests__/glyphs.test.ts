...
describe('supportsUnicode', () => {
  ...
  const NO_TERMINAL_SIGNALS: Record<string, string | undefined> = {
    AFYX_GRAPH_ASCII: undefined,
    AFYX_GRAPH_UNICODE: undefined,
    TERM: undefined,
    CI: undefined,
  ...
  it('AFYX_GRAPH_ASCII=1 still wins inside Windows Terminal (escape hatch)', () => {
    withEnv({ ...NO_TERMINAL_SIGNALS, AFYX_GRAPH_ASCII: '1', WT_SESSION: 'a-guid' }, () => {
      setPlatform('win32');
      expect(supportsUnicode()).toBe(false);
    });
  });
  ...
  describe('supportsUnicodeRawWrites', () => {
    ...
    it('AFYX_GRAPH_UNICODE=1 opts the raw path in on Windows', () => {
      withEnv({ ...NO_TERMINAL_SIGNALS, AFYX_GRAPH_UNICODE: '1' }, () => {
        setPlatform('win32');
  ...
  it('returns true on macOS by default', () => {
    withEnv({ AFYX_GRAPH_ASCII: undefined, AFYX_GRAPH_UNICODE: undefined, TERM: undefined }, () => {
      setPlatform('darwin');
      expect(supportsUnicode()).toBe(true);
    });
  });
  ...
  it('returns false on Linux kernel console (TERM=linux)', () => {
    withEnv({ AFYX_GRAPH_ASCII: undefined, AFYX_GRAPH_UNICODE: undefined, TERM: 'linux' }, () => {
      setPlatform('linux');
      expect(supportsUnicode()).toBe(false);
    });
  });

  it('respects AFYX_GRAPH_UNICODE=1 on Windows (opt-in escape hatch)', () => {
    withEnv({ AFYX_GRAPH_UNICODE: '1', AFYX_GRAPH_ASCII: undefined }, () => {
      setPlatform('win32');
      expect(supportsUnicode()).toBe(true);
    });
  });

  it('respects AFYX_GRAPH_ASCII=1 on macOS (opt-out escape hatch)', () => {
    withEnv({ AFYX_GRAPH_ASCII: '1', AFYX_GRAPH_UNICODE: undefined }, () => {
      setPlatform('darwin');
      expect(supportsUnicode()).toBe(false);
    });
  });

  it('AFYX_GRAPH_ASCII takes precedence over AFYX_GRAPH_UNICODE', () => {
    withEnv({ AFYX_GRAPH_ASCII: '1', AFYX_GRAPH_UNICODE: '1' }, () => {
      setPlatform('darwin');
      expect(supportsUnicode()).toBe(false);
    });
  });
...
describe('getGlyphs', () => {
  ...
  it('returns Unicode glyphs on macOS', () => {
    withEnv({ AFYX_GRAPH_ASCII: undefined, AFYX_GRAPH_UNICODE: undefined }, () => {
      setPlatform('darwin');
      const g = getGlyphs();
