 ...
 * the full real path: real Afyx Graph index + real ToolHandler.execute().
 *
...
describe('MCP staleness banner', () => {
  ...
  beforeEach(async () => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'afyx-graph-stale-banner-'));
    ...
    // files shared imports/calls, afyx_graph_search responses would surface
  ...
  it('prepends a stale banner when the response references a pending file', async () => {
    ...
    const res = await handler.execute('afyx_graph_search', { query: 'alphaOnly' });
  ...
  it('lists pending files under "Pending sync" in afyx_graph_status', async () => {
    cg.watch({ debounceMs: 4000, inertForTests: true });
    await cg.waitUntilWatcherReady();
  ...
  it('surfaces the degraded state as its own section in afyx_graph_status (#876)', async () => {
    degradeWatcher();
