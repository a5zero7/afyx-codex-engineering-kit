/**
 * #383 — Afyx Graph indexes config KEYS but must never surface config VALUES.
 *
 ...
 * passwords, API keys, JDBC URLs with embedded creds). Afyx Graph must surface
 * the KEY and never the value — not in node metadata (docstring/signature),
 * not via `afyx_graph_explore`'s verbatim source dump, and not via
 * `afyx_graph_node` `includeCode`. An agent that genuinely needs a value can
 * read the file itself (a deliberate pull); Afyx Graph must never volunteer it.
 */
...
describe('config secret redaction (#383)', () => {
  ...
  it('afyx_graph_explore surfaces the config key but NEVER the secret value', async () => {
    const res = await handler.execute('afyx_graph_explore', {
  ...
  it('afyx_graph_node includeCode returns the key, not the secret value', async () => {
    const res = await handler.execute('afyx_graph_node', {
