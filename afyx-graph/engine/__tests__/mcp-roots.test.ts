 ...
 * failed with a misleading "Afyx Graph not initialized. Run 'afyx-graph init'."
 *
...
describe('MCP project resolution via roots/list (issue #196)', () => {
  let cwdDir: string;     // where the server is launched — has NO .afyx-graph
  let projectDir: string; // the real indexed project the client reports
  let child: ChildProcessWithoutNullStreams | null = null;

  beforeEach(() => {
    cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'afyx-graph-mcp-cwd-'));
    projectDir = fs.mkdtempSync(path.join(os.tmpdir(), 'afyx-graph-mcp-proj-'));
  });
  ...
  it('resolves the project from the client roots/list when no rootUri is sent', async () => {
    const cg = await AfyxGraph.init(projectDir);
    cg.close();
    ...
    expect(text).toContain('Afyx Graph Status');
    expect(text).not.toContain('No Afyx Graph project is loaded');
  }, 20000);
  ...
  it('returns an actionable error when there is no rootUri and no roots capability', async () => {
    ...
    expect(text).toContain('No Afyx Graph project is loaded');
