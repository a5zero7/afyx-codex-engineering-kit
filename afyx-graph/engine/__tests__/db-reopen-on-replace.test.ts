 ...
 *
 * A long-lived process (the MCP daemon) opens `.afyx-graph/afyx-graph.db` and
 * holds the file descriptor for its whole life. If `.afyx-graph/` is removed and
 * recreated AT THE SAME PATH while it's running — `git worktree remove <p>` then
 * `git worktree add <p>` + `afyx-graph init`, or `rm -rf .afyx-graph` + re-init —
...
import { getAfyxGraphDir } from '../src/directory';
...
describe('AfyxGraph.reopenIfReplaced (issue #925)', () => {
  let root: string;
  ...
  posixOnly('heals a held connection after the index is removed and recreated at the same path', async () => {
    ...
    const server = AfyxGraph.initSync(root);
    ...
    // Simulate `git worktree remove` + re-add (or rm -rf .afyx-graph + init):
    // a NEW index inode at the same path, carrying a renamed symbol, written by
    // a separate instance (mirrors a fresh `afyx-graph init` process).
    fs.rmSync(getAfyxGraphDir(root), { recursive: true, force: true });
    fs.writeFileSync(path.join(root, 'src', 'a.ts'), 'export function fooNew() { return 2; }\n');
    const fresh = AfyxGraph.initSync(root);
