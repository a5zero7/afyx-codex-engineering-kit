...
describe('issue #238 — connection PRAGMAs (#1)', () => {
  ...
  beforeAll(() => {
    ...
    conn = DatabaseConnection.initialize(path.join(dir, 'afyx-graph.db'));
  });
...
describe('issue #238 — WAL lets a reader proceed during a writer', () => {
  ...
  it('a read on a 2nd connection succeeds while a writer holds the lock', () => {
    const dbPath = path.join(dir, 'afyx-graph.db');
...
describe('issue #238 — ToolHandler reuses the default instance (#2)', () => {
  ...
  it('getAfyxGraph(defaultRoot) returns the default instance, not a new connection', () => {
    const openSpy = vi.spyOn(AfyxGraph, 'openSync');
    try {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const resolved = (handler as any).getAfyxGraph(root);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const nested = (handler as any).getAfyxGraph(path.join(root, 'does', 'not', 'exist'));
  ...
  it('concurrent read tool calls (mixed projectPath) all succeed without "database is locked"', async () => {
    ...
    try {
      const calls: Promise<{ content: Array<{ text: string }>; isError?: boolean }>[] = [
        handler.execute('afyx_graph_search', { query: 'helper' }),
        handler.execute('afyx_graph_search', { query: 'helper', projectPath: root }),
        handler.execute('afyx_graph_callers', { symbol: 'helper', projectPath: root }),
        handler.execute('afyx_graph_callees', { symbol: 'main' }),
        handler.execute('afyx_graph_files', { projectPath: root }),
        handler.execute('afyx_graph_status', { projectPath: root }),
      ];
