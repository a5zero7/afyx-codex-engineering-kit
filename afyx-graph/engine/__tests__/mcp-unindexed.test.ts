 ...
 *
 * A server whose own root has no .afyx-graph/ still exposes its tools — gating
 ...
 * `afyx-graph init`. So `initialize` returns the per-project instructions
 ...
 * returns SUCCESS-shaped guidance ("pass projectPath / run afyx-graph init"),
 * never `isError: true` — one or two early isError responses teach an agent to
 * abandon afyx-graph for the whole session, and that failure mode is still
 * guarded below.
 */
...
describe('No-root-index session policy', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'afyx-graph-unindexed-'));
  });
  ...
  it('initialize returns the per-project instructions (not "inactive", not the full playbook)', async () => {
    ...
    expect(instructions).toMatch(/afyx_graph_explore/);
    expect(instructions).toMatch(/afyx-graph init/);
  ...
  it('tools/list exposes the tools even when the server root has no index (#964)', async () => {
    ...
    expect(tools.map((t) => t.name)).toContain('afyx_graph_explore');
  });
  ...
  it('a query by projectPath reaches an INDEXED sub-project of an unindexed root (monorepo) (#964)', async () => {
    ...
    // sub-project's .afyx-graph/ and return real results. Run through the real
    ...
    const cg = await AfyxGraph.init(svc, { index: true });
    cg.close();
    ...
    const res = await request(child, {
      id: 1,
      method: 'tools/call',
      params: { name: 'afyx_graph_search', arguments: { query: 'validateToken', projectPath: svc } },
    });
  ...
  it('an INDEXED workspace still gets the full playbook and the explore tool', async () => {
    ...
    const cg = await AfyxGraph.init(tempDir, { index: true });
    cg.close();
...
describe('No-error policy on expected conditions', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'afyx-graph-noerror-'));
  });
  ...
  it('cross-project query to an unindexed path is SUCCESS-shaped guidance, not isError', async () => {
    const res = await new ToolHandler(null).execute('afyx_graph_search', {
      query: 'anything',
      projectPath: tempDir,
    });
    ...
    expect(res.content[0]!.text).toMatch(/afyx-graph init/);
  ...
  it('no-default-project (working-directory detection miss) is SUCCESS-shaped guidance', async () => {
    const res = await new ToolHandler(null).execute('afyx_graph_search', { query: 'anything' });

    expect(res.isError).toBeUndefined();
    expect(res.content[0]!.text).toMatch(/No Afyx Graph project is loaded/);
...
describe('search kind filter', () => {
  ...
  beforeEach(async () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'afyx-graph-kind-'));
    fs.writeFileSync(
  ...
  it("kind: 'type' (the advertised enum value) finds type aliases", async () => {
    const res = await new ToolHandler(cg).execute('afyx_graph_search', {
      query: 'PaymentMethod',
      kind: 'type',
    });
