 ...
 *
 * Pinned because the lookup vocabulary is what makes afyx-graph useful
...
function tmpRoot(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'afyx-graph-symbol-lookup-'));
}
...
describe.skipIf(!HAS_SQLITE)('matchesSymbol — module-qualified lookups (#173)', () => {
  ...
  it('preserves afyx_graph_node file-basename lookup (#1473)', () => {
  ...
  it('afyx_graph_node with a `file` hint pins an overloaded name to that file', async () => {
...
describe.skipIf(!HAS_SQLITE)('matchesSymbol — dotted lookups (regression for #173 fix)', () => {
  ...
  it('afyx_graph_node on an ambiguous bare name returns ALL overloads with bodies (no guess)', async () => {
    ...
    const res = await handler.execute('afyx_graph_node', { symbol: 'request', includeCode: true });
    const text = res.content?.[0]?.text ?? '';
