 ...
 * AFYX_GRAPH_KERNEL_EXPECT=1, which turns "missing binary" into a FAILURE so
...
it.runIf(expectKernel)('kernel binary must exist when AFYX_GRAPH_KERNEL_EXPECT=1', () => {
...
describe.skipIf(!kernelBuilt)('kernel scaffold', () => {
  ...
  it('TS/JS family + Java + Python + Go route to the kernel by default; others stay wasm', () => {
    ...
    // AFYX_GRAPH_KERNEL_LANGS REPLACES the default set when present.
    process.env.AFYX_GRAPH_KERNEL_LANGS = 'tsx';
    expect(kernelRoutes('typescript')).toBe(false);
    expect(kernelRoutes('tsx')).toBe(true);
  });

  describe('with typescript routed (AFYX_GRAPH_KERNEL_LANGS)', () => {
    beforeEach(() => {
      process.env.AFYX_GRAPH_KERNEL_LANGS = 'typescript';
    });
    ...
    it('AFYX_GRAPH_KERNEL=0 kill switch disables routing', () => {
    ...
    it('tsx routes with its own entry and returns a graph', () => {
      process.env.AFYX_GRAPH_KERNEL_LANGS = 'typescript,tsx';
  ...
  describe('extractFromSource seam', () => {
    ...
    it('routed language takes the kernel and falls back per file on kernel absence', () => {
      ...
      process.env.AFYX_GRAPH_KERNEL_PATH = path.join(__dirname, 'nope', 'missing.node');
      process.env.AFYX_GRAPH_KERNEL = '0'; // and belt-and-braces the kill switch
      resetKernelForTests();
