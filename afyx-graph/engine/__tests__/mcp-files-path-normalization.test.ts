/**
 * afyx_graph_files path-filter normalization (#426)
 *
...
describe('afyx_graph_files path normalization', () => {
  let tempDir: string;
  let cg: AfyxGraph;
  let handler: ToolHandler;

  beforeEach(async () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'afyx-graph-files-paths-'));
  ...
  async function listed(pathFilter: string | undefined): Promise<string> {
    const result = await handler.execute('afyx_graph_files', {
