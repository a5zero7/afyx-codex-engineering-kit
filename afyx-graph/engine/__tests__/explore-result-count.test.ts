/**
 * afyx_graph_explore — the "Found N symbols across M files." header reflects the
...
describe('afyx_graph_explore — curated result count (#1046)', () => {
  let testDir: string;
  let cg: AfyxGraph;
  let handler: ToolHandler;

  beforeEach(async () => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'afyx-graph-count-'));
  ...
  it('header file count equals the number of rendered source sections', async () => {
    const res = await handler.execute('afyx_graph_explore', { query: 'publish status' });
    ...
    expect(text).not.toContain('afyx-graph-explore-summary');
  });
});
