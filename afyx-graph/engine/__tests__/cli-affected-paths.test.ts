/**
 * `afyx-graph affected` input-path normalization (#825).
 *
...
describe('afyx-graph affected — input path normalization (#825)', () => {
  let tempDir: string;

  beforeEach(async () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'afyx-graph-affected-paths-'));
