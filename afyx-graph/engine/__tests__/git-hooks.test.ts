...
describe('git sync hooks', () => {
  ...
  beforeEach(() => {
    repo = fs.mkdtempSync(path.join(os.tmpdir(), 'afyx-graph-githooks-'));
  });
  ...
  it('installs all default hooks, executable, invoking afyx-graph sync', () => {
    gitInit(repo);
    ...
    for (const hook of DEFAULT_SYNC_HOOKS) {
      ...
      expect(body).toContain('afyx-graph sync');
      expect(body).toContain('command -v afyx-graph'); // no-op when not on PATH
  ...
  it('is idempotent — re-install does not duplicate the block', () => {
    ...
    const occurrences = body.split('# >>> afyx-graph sync hook >>>').length - 1;
  ...
  it('remove keeps user content when the hook is shared', () => {
    ...
    expect(body).not.toContain('afyx-graph sync');
  });
