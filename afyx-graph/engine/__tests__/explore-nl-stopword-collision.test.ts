/**
 * afyx_graph_explore — NL-stopword collision guard (named-symbol seeding).
 *
...
describe('afyx_graph_explore — NL-stopword collision guard', () => {
  let testDir: string;
  let cg: AfyxGraph;
  let handler: ToolHandler;

  beforeEach(async () => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'afyx-graph-stopword-'));
  ...
  async function explore(query: string): Promise<string> {
    const res = await handler.execute('afyx_graph_explore', { query });
