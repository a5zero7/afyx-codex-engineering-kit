 ...
 * committed `afyx-graph.json` at the repo root, so files that would otherwise be
 ...
 * map, the loader's validation/normalization/caching of `afyx-graph.json`, and a
...
describe('custom extension → language mapping (#906)', () => {
  ...
  describe('loadExtensionOverrides (afyx-graph.json)', () => {
    let dir: string;
    beforeEach(() => {
    ...
    it('returns an empty map when there is no afyx-graph.json', () => {
  ...
  describe('indexAll honors afyx-graph.json end-to-end', () => {
    let dir: string;
    beforeEach(() => {
    ...
    it('indexes a custom-extension file mapped to a supported language', async () => {
      write('afyx-graph.json', JSON.stringify({ extensions: { '.foo': 'typescript' } }));
    ...
    it('does NOT index the same file without afyx-graph.json (zero-config preserved)', async () => {
      write('widget.foo', SOURCE);
