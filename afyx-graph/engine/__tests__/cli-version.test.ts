/**
 * Tests for the `afyx-graph version` affordances.
 *
...
describe('afyx-graph version affordances', () => {
  for (const spelling of ['version', '-v', '-version', '--version', '-V']) {
    it(`\`afyx-graph ${spelling}\` prints exactly the package version`, () => {
  ...
  it('`afyx-graph help` prints usage and the command list', () => {
    const out = run(['help']);
    expect(out).toContain('Usage: afyx-graph');
  ...
  it('a trailing `-v` is still the subcommand\'s --verbose, not the version intercept', () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'afyx-graph-version-test-'));
    let combined = '';
    try {
