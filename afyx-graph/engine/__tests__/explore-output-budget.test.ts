/**
 * Adaptive output budget for afyx_graph_explore (#185).
 *
 ...
 */
describe('afyx_graph_explore output respects the adaptive budget', () => {
  let testDir: string;
  let cg: AfyxGraph;
  let handler: ToolHandler;

  beforeAll(async () => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'afyx-graph-explore-budget-'));
  ...
  it('keeps total output under the small-project cap', async () => {
    const result = await handler.execute('afyx_graph_explore', { query: 'Session method helper' });
  ...
  it('emits advisory-only exploration guidance on medium projects — never quota wording', async () => {
    ...
    const spy = vi.spyOn(cg, 'getStats').mockReturnValue({ fileCount: 1000 } as ReturnType<AfyxGraph['getStats']>);
    try {
      const result = await handler.execute('afyx_graph_explore', { query: 'Session method helper' });
      const text = result.content?.[0]?.text ?? '';
  ...
  it('prefixes source lines with line numbers by default (cat -n style)', async () => {
    delete process.env.AFYX_GRAPH_EXPLORE_LINENUMS;
    const result = await handler.execute('afyx_graph_explore', { query: 'Session method helper' });
    const text = result.content?.[0]?.text ?? '';
  ...
  it('omits line numbers when AFYX_GRAPH_EXPLORE_LINENUMS=0', async () => {
    process.env.AFYX_GRAPH_EXPLORE_LINENUMS = '0';
    try {
      const result = await handler.execute('afyx_graph_explore', { query: 'Session method helper' });
      const text = result.content?.[0]?.text ?? '';
