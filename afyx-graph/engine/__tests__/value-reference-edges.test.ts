 ...
 * "change this constant, affect its readers". Default on; AFYX_GRAPH_VALUE_REFS=0
...
function valueRefReaders(cg: AfyxGraph, constName: string): string[] {
...
describe('value-reference edges', () => {
  ...
  beforeEach(() => {
    dir = fs.mkdtempSync(path.join(os.tmpdir(), 'afyx-graph-valueref-'));
  });
  afterEach(() => {
    cg?.destroy();
    cg = undefined;
  ...
  function index(): AfyxGraph {
    const g = AfyxGraph.initSync(dir, { config: { include: ['**/*.ts', '**/*.tsx'], exclude: [] } });
    return g;
  }
  ...
  it('emits nothing when AFYX_GRAPH_VALUE_REFS=0', async () => {
    const prev = process.env.AFYX_GRAPH_VALUE_REFS;
    process.env.AFYX_GRAPH_VALUE_REFS = '0';
    try {
      fs.writeFileSync(
        path.join(dir, 'config.ts'),
      ...
      if (prev === undefined) delete process.env.AFYX_GRAPH_VALUE_REFS;
      else process.env.AFYX_GRAPH_VALUE_REFS = prev;
    }
  });
});
