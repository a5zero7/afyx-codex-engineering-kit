...
import { AfyxGraph } from '../../src/index.js';
...
if (!fs.existsSync(path.join(resolvedPath, '.afyx-graph', 'afyx-graph.db'))) {
  console.error(`No .afyx-graph/afyx-graph.db found at ${resolvedPath}`);
  process.exit(1);
}

let afyxGraphSha = 'unknown';
try {
  afyxGraphSha = execSync('git rev-parse --short HEAD', { encoding: 'utf-8' }).trim();
} catch {}

console.log(`\nAfyxGraph Eval — ${path.basename(resolvedPath)}`);
console.log(`Codebase: ${resolvedPath}`);
console.log(`Commit:   ${afyxGraphSha}`);
console.log(`Cases:    ${testCases.length}`);
console.log('');

async function run() {
  const cg = AfyxGraph.openSync(resolvedPath);
