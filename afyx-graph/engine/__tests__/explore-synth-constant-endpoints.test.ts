/**
 * Regression: afyx_graph_explore must SURFACE a synthesized edge whose endpoints are
...
// Assertions read RAW afyx_graph_explore output; managed offload would replace it. Disable
...
beforeAll(() => { _prevOffloadDisable = process.env.AFYX_GRAPH_OFFLOAD_DISABLE; process.env.AFYX_GRAPH_OFFLOAD_DISABLE = '1'; });
afterAll(() => {
  if (_prevOffloadDisable === undefined) delete process.env.AFYX_GRAPH_OFFLOAD_DISABLE;
  else process.env.AFYX_GRAPH_OFFLOAD_DISABLE = _prevOffloadDisable;
});

describe('afyx_graph_explore — synthesized constant→constant edges surface on small repos', () => {
  let dir: string;
  let cg: AfyxGraph;
...
`
    ...
    const res = await handler.execute('afyx_graph_explore', { query: 'outerThunk innerThunk' });
