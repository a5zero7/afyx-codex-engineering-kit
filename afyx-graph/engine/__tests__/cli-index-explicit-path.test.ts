/**
 * `afyx-graph index <path>` rebuilds <path>, never an ancestor (#1524).
 *
 ...
 * `afyx-graph index child` from a monorepo re-indexed the whole container and
...
describe('afyx-graph index <path> (#1524)', () => {
  let root: string;
  let parent: string;
  let child: string;

  beforeAll(async () => {
    root = fs.mkdtempSync(path.join(os.tmpdir(), 'afyx-graph-index-path-'));
    ...
    const cg = AfyxGraph.initSync(parent);
    await cg.indexAll();
    cg.close();
  });
  ...
  it('refuses an explicit path that has no index of its own, naming the ancestor it would have rebuilt', () => {
    const before = fs.statSync(path.join(parent, '.afyx-graph', 'afyx-graph.db')).mtimeMs;
    ...
    expect(fs.statSync(path.join(parent, '.afyx-graph', 'afyx-graph.db')).mtimeMs).toBe(before);
    expect(fs.existsSync(path.join(child, '.afyx-graph'))).toBe(false);
  });
