 ...
 * DatabaseConnection / Afyx Graph report it and come up in WAL.
 */
...
describe('DatabaseConnection — backend reporting', () => {
  ...
  beforeEach(() => {
    dir = fs.mkdtempSync(path.join(os.tmpdir(), 'afyx-graph-backend-'));
  });
  ...
  it('AfyxGraph.getBackend() delegates to the underlying DatabaseConnection', async () => {
