...
describe('watchDisabledReason', () => {
  it('returns a reason when AFYX_GRAPH_NO_WATCH=1', () => {
    const reason = watchDisabledReason('/home/me/project', {
      env: { AFYX_GRAPH_NO_WATCH: '1' },
      isWsl: false,
    });
  ...
  it('AFYX_GRAPH_FORCE_WATCH=1 overrides WSL auto-detect', () => {
  ...
  it('AFYX_GRAPH_NO_WATCH wins over AFYX_GRAPH_FORCE_WATCH', () => {
    const reason = watchDisabledReason('/home/me/project', {
...
describe('FileWatcher honors the watch policy', () => {
  ...
  afterEach(() => {
    delete process.env.AFYX_GRAPH_NO_WATCH;
    if (testDir && fs.existsSync(testDir)) {
      fs.rmSync(testDir, { recursive: true, force: true });
    }
  });

  it('does not start when AFYX_GRAPH_NO_WATCH=1', () => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'afyx-graph-nowatch-'));
