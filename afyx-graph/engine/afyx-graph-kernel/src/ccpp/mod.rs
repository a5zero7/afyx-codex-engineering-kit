...
//!    AFYX_GRAPH_VALUE_REFS=0 kill switch.
...
pub fn extract(file_path: &str, source: &str, language: &str) -> Result<EmitOut, String> {
    ...
    let no_defer = std::env::var("AFYX_GRAPH_KERNEL_CCPP_ERROR_EXTRACT").as_deref() == Ok("1");
...
impl<'t> Walker<'t> {
    ...
    fn flush_value_refs(&mut self, root: Node<'t>) {
        ...
        if std::env::var("AFYX_GRAPH_VALUE_REFS").as_deref() == Ok("0") {
            return;
        }
