...
refs_ok() {
  local name="$1"; shift
  local reference
  for reference in "$@"; do [[ -f "$skills_root/$name/$reference" ]] || return 1; done
}
...
else result FAIL 'Codex CLI' 'not found'; core_failure=true; fi
...
if skill_ok odoo-engineering && refs_ok odoo-engineering "${odoo_refs[@]}"; then result OK 'Odoo Engineering (10-20)' 'frontmatter and required references valid'; else result FAIL 'Odoo Engineering (10-20)' 'SKILL.md or required reference invalid'; core_failure=true; fi
...
for component in codegraph headroom; do
